...
//					break;
        ...
        private Rectangle lastMouseRect = Rectangle.Empty;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (this._StyleProvider.ShowTabCloser)
            {
                ...
                for (int index = this.TabCount - 1; index >= 0; index--) {
                    ...
                    if (closerRect.Contains(this.MousePosition) && lastMouseRect != closerRect)
                    {
                        Console.WriteLine(1);
                        lastMouseRect = closerRect;
                        this.Invalidate();                     
                        break;
                    }
                }
                if (!lastMouseRect.Contains(this.MousePosition) && lastMouseRect != Rectangle.Empty)
                {
                    lastMouseRect = Rectangle.Empty;
                    this.Invalidate();
                }
...
//		}
		...
		private void CustomPaint(Graphics screenGraphics){
			...
			if (this.Width > 0 && this.Height > 0){
                if (this._BackImage == null)
                {
                    ...
                    backGraphics.Clear(Color.FromArgb(160, 160, 160));
        ...
        private void DrawNewTab(Graphics graphics)
        {
            graphics.SmoothingMode = SmoothingMode.HighQuality;
            Rectangle r = GetTabRect(this.TabCount-1);
            r.X += r.Width + 3;
            r.Y += 3;
            r.Width = 24;
            r.Height = r.Height - 6;

            Point[] p =  { new Point(r.X-6, r.Y), new Point(r.X + r.Width-6, r.Y),  new Point(r.X + r.Width, r.Y + r.Height),new Point(r.X, r.Y + r.Height) };
            Pen pen = new Pen(System.Drawing.ColorTranslator.FromHtml("#607082"), 2);
            Brush brush = new SolidBrush(System.Drawing.ColorTranslator.FromHtml("#a3c8f4"));
            if (r.Contains(MousePosition))
            {
               
                pen = new Pen(System.Drawing.ColorTranslator.FromHtml("#407082"), 2);
                brush = new SolidBrush(System.Drawing.ColorTranslator.FromHtml("#b3c8f4"));
